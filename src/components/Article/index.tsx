...
export default function Article({ slug = '' }: RouteComponentProps<{ slug: string }>) {
  const [{ article, comments, loading, error }, dispatch] = React.useReducer(articleReducer, initialState)
  React.useEffect(() => {
    ...
    const fetchArticle = async () => {
      try {
        const [articlePayload, commentsPayload] = await Promise.all([getArticle(slug), getArticleComments(slug)])
        if (!ignore) {
          dispatch({
